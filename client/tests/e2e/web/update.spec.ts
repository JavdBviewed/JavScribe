...
const cur = async (req: APIRequestContext) =>
  ((await (await req.get(`${WEB_URL}/api/health`)).json()) as { version: string }).version;
...
test.beforeEach(async ({ page, request }) => {
  ...
  await ghReset(request); // mock-github 恢复默认（= 当前工作台版本）
  await page.goto("/");
...
test("服务落后（工作台同版本）：同样提示更新", async ({ page }) => {
  // 工作台 current 与默认镜像版本持平，但已登记服务上报 0.0.1 → 仍提示更新
  const v = await cur(req);
